...
- Frontend: `pnpm dev` · `pnpm build` · `pnpm type-check` (`vue-tsc --noEmit`) · `pnpm test` (vitest 3.x, `tests/**/*.test.ts`,74 用例跨 6 个 `.test.ts` 文件). First test file: `tests/views/order/constants.test.ts` (15 用例覆盖 ORDER_STATUS 5 态 + 跨态 round-trip).
- Backend: `mvn spring-boot:run` · `mvn test` (105 JUnit5+Mockito unit tests across 24 classes,2026-09-21 实测;早期数字 29/8 → 69/17 过期).
